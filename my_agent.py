...
class MyAgent:
    ...
    def build_state(self, state: dict) -> np.ndarray:
        ...
        bird_velocity = (state['bird_velocity'] + 7) / 31  # Normalize bird's velocity (0-1)
        ...
        if pipe_count > 0:
            ...
            x_distance = 1
            y_distance = (state['bird_y'] + state['bird_height']/2) / screen_height
            y_distance2 = y_distance

        phi = np.array([bird_velocity, x_distance, y_distance, y_distance2])
    ...
    def reward(self, state: dict, phi: dict) -> float:
        if self.next_pipe:
            if state['bird_y'] < self.next_pipe['top']:
                ...
                r_centre = -(state['bird_y'] + state['bird_height'] - self.next_pipe['bottom']) / state['screen_height']
                ...
                if state['bird_y'] < self.next_pipe2['top']:
                    ...
                    r_centre = -(state['bird_y'] + state['bird_height'] - self.next_pipe2['bottom']) / state['screen_height'] * 0.5
                else:
                    r_centre = 0
            else:
                r_centre = 0
        else:  r_centre = -abs(phi[2] - 0.5)/5

        # r_centre = -abs(phi[2] - 0.5)/5 - abs(phi[3] - 0.5)/10
        # r_centre = 0
        r_mileage = state['mileage']/4188
        ...
        if state['done']:
            ...
            elif state['done_type'] == 'hit_pipe': r_total = -0.4 + r_centre + r_mileage
            elif state['done_type'] == 'offscreen': r_total = -0.5 + r_centre + r_mileage
            else: 
...
import os
def save_epsilon_to_txt(epsilon, filename="epsilon.txt"):
    try:
...
def load_epsilon_from_txt(filename="epsilon.txt", default_epsilon=1.0):
...
if __name__ == '__main__':
    ...
    agent.epsilon = load_epsilon_from_txt()
    ...
    save_epsilon_to_txt(agent.epsilon)
    ...
    # plt.xlabel('Episode')
    # plt.ylabel('Mileage')
    # plt.title('Training Mileage vs. Episode')
    # plt.legend()
    # plt.grid(True)
    # # Plot 3:
    # plt.subplot(2, 2, 3)  # 1 row, 2 columns, second plot
    # plt.plot(range(len(agent.epsilons)), agent.epsilons, label='Training Epsilons', marker='o', linewidth=0.5, markersize=0.5, mfc = 'r')
    # plt.xlabel('Epsilons')
    # plt.ylabel('Decays')
    ...
    # plt.scatter(range(len(rewards)), rewards, label='rewards in storage', s=10)
    # plt.xlabel('Actions')
    # plt.ylabel('Reward')
    # plt.title('Reward vs. Actions')
    # plt.legend()
    # plt.grid(True)
